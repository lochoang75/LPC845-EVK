...
uint32_t GetConsoleInput(uint32_t number_of_digits) {
  ...
  printf("\rEnter a %d-digit hex number (with no prefix): \n\r", number_of_digits);
  ...
  while(n--)
  {
    temp |= num[n]<<(4*((number_of_digits-1)-n)); // Reverse the order in which digits are received // Dao nguoc thu tu so da nhap
  }
